//1 Создайте функцию uniqueValues, которая принимает массив 
//значений и возвращает новый массив, содержащий только 
//уникальные значения из входного массива.
...
//2 Реализуйте класс Storage, который будет использовать 
//Map для хранения данных. Класс должен предоставлять методы 
//setItem для добавления данных, getItem для извлечения данных 
//по ключу и removeItem для удаления данных по ключу.
...
//3 Реализуйте функцию memoize, которая принимает функцию 
//func и возвращает новую функцию, которая кэширует 
//результаты вызовов func для уникальных аргументов. 
//Используйте Map для хранения результатов и WeakMap 
//для хранения ссылок на объекты, чтобы избежать утечек памяти.
...
function trackUniqueObjectsWithWeakSet(objects) {
  ...
  objects.forEach(el => {
    workSet.add(JSON.stringify(el));
  });

  for(let el of workSet) {
    el = JSON.parse(el);
    arr.push(el);
...
console.dir(uniqueObjectsSet); // value: {id: 2}
